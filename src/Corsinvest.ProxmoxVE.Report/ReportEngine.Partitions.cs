...
public partial class ReportEngine
{
    ...
    private int WritePartitionData(XLWorkbook workbook)
    {
        ...
        var rows = _pendingPartitionRows
            .SelectMany(entry => entry.Partitions.Select(a => new
            {
                entry.Vm.Node,
                entry.Vm.VmId,
        ...
        var partitionSw = CreateSheetWriter(workbook, "Partitions");
        partitionSw.CreateTable(null,
                                rows,
                                tbl =>
                                {
                                    partitionSw.ApplyNodeLinks(tbl);
                                    partitionSw.ApplyVmIdLinks(tbl);
                                });
        partitionSw.AdjustColumns();

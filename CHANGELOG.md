...
- **Compliance section — ISO 27001, NIS2, CIS v8, AgID, PCI DSS, GDPR, DORA, NIST CSF 2.0, ISO 27017.** Opt-in section that audits the cluster state against 9 widely-adopted security and regulatory frameworks. Each enabled standard gets a dedicated page/sheet/file with: an **Info** block (controls, findings, score), a **Disclaimer**, a **Controls** table (one row per control with status PASS / FAIL / PARTIAL / N/A, score %, highest severity, findings count) and a **Checks** table (one row per check outcome with severity, scope, title, details and remediation). HTML uses a sidebar `Compliance` group with sub-pages under `compliance/`; Excel uses one `Compliance <PackId>` worksheet per standard plus a `Compliance` overview with hyperlinks; JSON ships `compliance/<pack>.json` files. Status renders as a coloured badge (✓ / ✗ / ◐ / —) and scores as a green-yellow-red colour scale; both columns are natively filterable in HTML and Excel. Enable in `settings.json` under `"Compliance"` or run `--full` to enable every standard. **The assessment is automated and technical only — procedural, organisational and physical controls are out of scope and the report does not constitute formal certification.** Full reference: [docs/compliance.md](docs/compliance.md).
